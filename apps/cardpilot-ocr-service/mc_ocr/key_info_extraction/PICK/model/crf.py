...
def _viterbi_decode(tag_sequence, transitions):
    path_scores = tag_sequence[0]
    backpointers = []
    for timestep in range(1, tag_sequence.size(0)):
        scores = path_scores.unsqueeze(1) + transitions
        best_scores, best_paths = scores.max(0)
        path_scores = best_scores + tag_sequence[timestep]
        backpointers.append(best_paths)
    best_score, best_last = path_scores.max(0)
    path = [int(best_last.item())]
...
class ConditionalRandomField(torch.nn.Module):
    ...
    def viterbi_tags(self,
        ...
        for prediction, prediction_mask in zip(logits, mask):
            ...
            viterbi_path, viterbi_score = _viterbi_decode(

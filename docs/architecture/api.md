...
The first read-only mobile bootstrap slice is implemented:

| Method | Path                                              | Purpose                                                      |
| ------ | ------------------------------------------------- | ------------------------------------------------------------ |
| GET    | `/api/v1/merchant-category-codes`                 | Full active MCC reference list                               |
| GET    | `/api/v1/credit-cards/:creditCardId/reward-rules` | Active reward rules plus MCC mappings for one card product   |
| GET    | `/api/v1/merchants/mcc-suggestions?query=...`     | Merchant/branch MCC candidates with confidence and status    |
| GET    | `/api/v1/merchants`                               | Complete merchant-branch directory with MCC/payment mappings |

Merchant search never assumes one MCC per brand. Multiple merchant rows may
share a normalized name while carrying different locations, and each merchant
may have multiple MCC candidates. The transaction stores the MCC finally
confirmed by the user.

`GET /api/v1/merchants` returns one item per cloud merchant/branch. Mobile
groups rows by `nameNormalized` for brand-level navigation. Every candidate
contains a `paymentType` (`in_store`, `online`, `shopee_food`, `grab_food`,
`other`, or `unknown`) so direct and delivery-platform payments can retain
different MCC mappings. User contributions are local-only in the current
mobile phase; there is no write API for them yet.
